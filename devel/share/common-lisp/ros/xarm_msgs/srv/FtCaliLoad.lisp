...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<FtCaliLoad-request>)))
  ...
  (cl:format cl:nil "# This format is suitable for the following services~%#   - set_ft_sensor_load_offset     # The old name is ft_sensor_cali_load, please use the new one~%~%# iden result([mass(kg)，x_centroid(mm)，y_centroid(mm)，z_centroid(mm)，Fx_offset，Fy_offset，Fz_offset，Tx_offset，Ty_offset，Tz_ffset])~%float32[] datas~%~%# whether to convert the paramster to the corresponding tcp load and set~%bool association_setting_tcp_load~%# float32 m   0.270~%# float32 x   -17.0~%# float32 y   9.0~%# float32 z   11.8~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'FtCaliLoad-request)))
  "Returns full string definition for message of type 'FtCaliLoad-request"
  (cl:format cl:nil "# This format is suitable for the following services~%#   - set_ft_sensor_load_offset     # The old name is ft_sensor_cali_load, please use the new one~%~%# iden result([mass(kg)，x_centroid(mm)，y_centroid(mm)，z_centroid(mm)，Fx_offset，Fy_offset，Fz_offset，Tx_offset，Ty_offset，Tz_ffset])~%float32[] datas~%~%# whether to convert the paramster to the corresponding tcp load and set~%bool association_setting_tcp_load~%# float32 m   0.270~%# float32 x   -17.0~%# float32 y   9.0~%# float32 z   11.8~%~%~%~%"))
